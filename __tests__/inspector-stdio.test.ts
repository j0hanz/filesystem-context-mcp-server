...
describe('Inspector CLI: Stdio Integration & Conformance', { skip: inspectorSkipReason() }, () => {
  ...
  it('INSP-STDIO-009: tools/call out-of-boundary path traversal returns ACCESS_DENIED', async () => {
    ...
    // Exit 5, not 0: the denial is a tool-level `isError` result — the correct
    // shape, since an out-of-root path is a bad argument and not a protocol
    // fault — and the Inspector CLI maps every `isError: true` onto exit 5
    // (`tool_is_error`). The structured payload below is what proves the server
    // answered ACCESS_DENIED rather than failing some other way.
    assert.strictEqual(
      res.exitCode,
      5,
