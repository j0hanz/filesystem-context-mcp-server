...
function registerSearchFilesBasics(getTestDir: () => string): void {
...
function registerSearchFilesLimits(getTestDir: () => string): void {
...
function registerSearchFilesErrors(getTestDir: () => string): void {
  void it('searchFiles rejects file base path', async () => {
    const filePath = path.join(getTestDir(), 'README.md');
    await assert.rejects(searchFiles(filePath, '**/*.ts'), /not a directory/i);
  });
}

void describe('searchFiles', () => {
  withFileOpsFixture((getTestDir) => {
    registerSearchFilesBasics(getTestDir);
    registerSearchFilesLimits(getTestDir);
    registerSearchFilesErrors(getTestDir);
  });
});
